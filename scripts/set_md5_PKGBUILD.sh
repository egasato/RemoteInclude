#!/usr/bin/env bash

# Sets the MD5 hash used inside PKGBUILD
function set_md5_PKGBUILD() {
    local __filename
    if [[ "${BASH_SOURCE[0]}" == "" ]]; then
        __filename=$(cd "$(dirname "$0")" && pwd -P)/$(basename "$0")
    else
        __filename=$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd -P)/$(basename "${BASH_SOURCE[0]}")
    fi
    local __dirname=$(dirname "$__filename")
    gawk -v project_md5="$1" -v project_name="$2" -v project_version="$3" -f "$__dirname/set_md5_PKGBUILD.gawk" "$__dirname/../PKGBUILD" > "$__dirname/../PKGBUILD.tmp"
    mv "$__dirname/../PKGBUILD.tmp" "$__dirname/../PKGBUILD"
}
