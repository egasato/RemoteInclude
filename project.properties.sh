...
# This shit shouldn't exist because I'm basically hard-coding the variables names
# and the only reason this script exists is to avoid that, but I'm tired of seeing
# warnings and errors in my IDE, so here we go
declare CMAKE_BUILD_TYPE
declare _AUTHOR_USER _AUTHOR_NAME _AUTHOR_NAME_ASCII _AUTHOR_EMAIL _PROJECT_NAME _PROJECT_SUMMARY _PROJECT_DESCRIPTION _PROJECT_VERSION _PROJECT_LICENSE
declare _AUTHOR_CONTACT _AUTHOR_CONTACT_ASCII
declare _PROJECT_NAME_LOWER _PROJECT_LICENSE_PKG _PROJECT_LICENSE_APK _PROJECT_VENDOR _PROJECT_VENDOR_ASCII _PROJECT_HOMEPAGE _PROJECT_URL _PROJECT_VCS _PROJECT_SOURCE _PROJECT_MD5 _PROJECT_SHA1 _PROJECT_SHA224 _PROJECT_SHA256 _PROJECT_SHA384 _PROJECT_SHA512
declare _LABEL_BUILD_DATE _LABEL_NAME _LABEL_DESCRIPTION _LABEL_USAGE _LABEL_URL _LABEL_VCS_URL _LABEL_VENDOR _LABEL_VERSION _LABEL_VCS_REF
...
[[ "$_PROJECT_NAME_LOWER"   == "" ]] && _PROJECT_NAME_LOWER=$(echo "$_PROJECT_NAME" | tr 'A-Z' 'a-z' | tr -s ' ' | tr ' ' '-')
[[ "$_PROJECT_LICENSE_PKG"  == "" ]] && _PROJECT_LICENSE_PKG=$(echo "$_PROJECT_LICENSE" | sed -r 's/(v|\.0)//g')
[[ "$_PROJECT_LICENSE_APK"  == "" ]] && _PROJECT_LICENSE_APK=$(echo "$_PROJECT_LICENSE" | sed -r 's/\.0//g')
...
[[ "$_LABEL_DESCRIPTION" == "" ]] && _LABEL_DESCRIPTION="$_PROJECT_SUMMARY"
...
[[ "$_CMAKE_BUILD_TYPE"  == "" ]] && _CMAKE_BUILD_TYPE="Release"
